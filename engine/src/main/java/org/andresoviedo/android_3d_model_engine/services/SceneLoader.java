...
public class SceneLoader implements LoadListener, EventListener {
    ...
    private boolean selectedObjMoved = false;
    private boolean clearSelectionsFlag = false;

    private Mode currentMode = Mode.WIRE_OUT;
    ...
    public boolean isInitialPosObtained() {
        return initialPosObtained;
    }

    public void setInitialPosObtained(boolean initialPosObtained) {
        this.initialPosObtained = initialPosObtained;
    }

    public void setObjects(List<Object3DData> objects) {
    ...
    private boolean initialPosObtained = false;
...
//            animateCamera();
        ...
        if (doAnimation) {
            for (int i = 0; i < getObjects().size(); i++) {
                Object3DData obj = getObjects().get(i);
    ...
    public final void showSettingsDialog() {
        ...
        if (getObjects().get(0) instanceof AnimatedModel) {
            animatedModel = (AnimatedModel) getObjects().get(0);
        } else return;
    ...
    public boolean isSelectedObjMoved() {
        return selectedObjMoved;
    }

    public void setSelectedObjMoved(boolean selectedObjMoved) {
        this.selectedObjMoved = selectedObjMoved;
    }

    public boolean isClearSelectionsFlag() {
        return clearSelectionsFlag;
    }

    public void setClearSelectionsFlag(boolean clearSelectionsFlag) {
        this.clearSelectionsFlag = clearSelectionsFlag;
    }

    @RequiresApi(api = Build.VERSION_CODES.N)
    public void initializeObjectController(Object3DData object3DData) {
        Log.i(getClass().getName(), "Loading ObjectController...");
        if (getObjectController() != null){
            setObjectController(null);
            ((ModelSurfaceView) glView).getModelRenderer().removeListener();
            ((ModelSurfaceView) glView).getTouchController().removeListener();
        }
        setObjectController(new ObjectController(object3DData));
        ((ModelSurfaceView) glView).getModelRenderer().addListener(getObjectController());
        ((ModelSurfaceView) glView).getTouchController().addListener(getObjectController());
    }
}
