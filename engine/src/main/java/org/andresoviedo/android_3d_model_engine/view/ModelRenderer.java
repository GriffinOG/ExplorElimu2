...
public class ModelRenderer implements GLSurfaceView.Renderer {
    ...
    private boolean boundingBoxDrawn = false;
    private Object3DData currentSelection;
    private Object3DData[] droppedObjects;
    ...
    public ModelRenderer removeListener(){
        Predicate<EventListener> ifObjectController = objCtrl -> (objCtrl instanceof ObjectController);
        this.listeners.removeIf(ifObjectController);
        return this;
    }
    ...
    private void onDrawFrame(float[] viewMatrix, float[] projectionMatrix, float[] viewProjectionMatrix,
        ...
        boolean hasSelectedObject = false;
        if (scene.getSelectedObject() != null)
            hasSelectedObject = true;

        // draw all available objects
        List<Object3DData> objects = scene.getObjects();
        if (!hasSelectedObject || !scene.getCurrentMode().equals(SceneLoader.Mode.WIRE_OUT)) {
            ...
            scene.setClearSelectionsFlag(false);
        } else {
            for (int i = 0; i < objects.size(); i++) {
                if (scene.getSelectedObject() == objects.get(i))
                    drawObject(viewMatrix, projectionMatrix, lightPosInWorldSpace, colorMask, cameraPosInWorldSpace, doAnimation, drawLighting, false, drawTextures, drawColors, objects, i);
                else
                    drawObject(viewMatrix, projectionMatrix, lightPosInWorldSpace, colorMask, cameraPosInWorldSpace, doAnimation, drawLighting, true, drawTextures, drawColors, objects, i);
            }
            scene.setClearSelectionsFlag(false);
        }
...
//            AndroidUtils.fireEvent(listeners, new FPSEvent(this, framesPerSecond));
    ...
    private void drawObject(float[] viewMatrix, float[] projectionMatrix, float[] lightPosInWorldSpace, float[] colorMask, float[] cameraPosInWorldSpace, boolean doAnimation, boolean drawLighting, boolean drawWireframe, boolean drawTextures, boolean drawColors, List<Object3DData> objects, int i) {
        ...
        try {
            ...
            } else if (objData.isMoved() && scene.isClearSelectionsFlag()){
                takeBack(objData);
            }
//            else if (scene.isExploded()){
//                moveAway(objData);
//            }
...
//        Camera camera = scene.getCamera();
        ...
        if (boundingBoxData == null || changed) {
            Log.i("ModelRenderer", "Building bounding box... id: " + objData.getId()
                    + " New location: "+objData.getLocationX()+","+objData.getLocationY()+","+objData.getLocationZ());
    ...
    private void isolate(Object3DData objData){
        if (!objData.isMoved()){
            objData.setInitialLoc(objData.getLocation());
            bringForth(objData);
            if (currentSelection != null) {
                takeBack(currentSelection);
            }
            currentSelection = objData;
        }

//        float centerX = (objData.getBoundingBox().getxMax() - objData.getBoundingBox().getxMin())/2;
//        float centerY = (objData.getBoundingBox().getyMax() - objData.getBoundingBox().getyMin())/2;
//        float centerZ = (objData.getBoundingBox().getzMin() - objData.getBoundingBox().getzMin())/2;
//
//        float[] centerLoc = new float[]{centerX, centerY, centerZ};
//        if (scene.getObjectController() != null && scene.getObjectController().getRotateBy() != null){
//            float [] rotateBy = scene.getObjectController().getRotateBy();
//            objData.setRotation2(rotateBy, centerLoc);
//        }
    }

    private void bringForth(Object3DData object3DData){
        final float initialX = object3DData.getLocationX();
        final float initialY = object3DData.getLocationY();
        final float initialZ = object3DData.getLocationZ();

        initialPos = new float[]{initialX, initialY, initialZ};
        Log.d(getClass().getName(), "Initial x:"+ initialX + ", Initial y:" + initialY + ", Initial z" + initialZ);

//        float transX = initialX+(30000f*(initialX));
        float transY = initialY+(25f*(initialY));
//        float transZ = initialZ+(.03f*(initialZ));

//        object3DData.setRotation()
        object3DData.setLocation(new float[]{initialX, transY, initialZ});
        object3DData.setRotation1(new float[]{40f, 0, 100f});
        object3DData.setMoved(true);
    }

    private void takeBack(Object3DData object3DData) {
        object3DData.setLocation(object3DData.getInitialLoc());
        object3DData.setMoved(false);
    }

    private void drop(Object3DData object3DData){
        final float initialX = object3DData.getLocationX();
        final float initialY = object3DData.getLocationY();
        final float initialZ = object3DData.getLocationZ();

        if (!scene.isInitialPosObtained()){
            object3DData.setInitialLoc(new float[]{initialX, initialY, initialZ});
            scene.setInitialPosObtained(true);
        }

        Log.d(getClass().getName(), "Initial x:"+ initialX + ", Initial y:" + initialY + ", Initial z:" + initialZ);

//        float transX = initialX+(30000f*(initialX));
//        float transY = initialZ+(.3f*(initialY));
        float transZ = initialZ+(.8f*(initialZ));

//        object3DData.setRotation()
        object3DData.setLocation(new float[]{initialX, initialY, transZ});
        object3DData.setMoved(true);
    }
    ...
    private void explode() {
        if (!scene.isExploded()){
            for (Object3DData object3DData : scene.getObjects()){
                if (object3DData.isMoved())
                    takeBack(object3DData);

                final float initialZ = object3DData.getLocationZ();
                float transZ;
                if (object3DData.getLocationZ() < 0)
                    transZ  = initialZ - (10f*(initialZ));
                else
                    transZ  = initialZ + (10f*(initialZ));

                object3DData.setLocation(new float[]{object3DData.getLocationX(), object3DData.getLocationY(), transZ});
                scene.setExploded(true);
            }
        }
    }

//    private void moveAway(Object3DData object3DData){
//        if (!object3DData.isMoved()){
//            final float initialY = object3DData.getLocationY();
//            float transY  = (float) Math.pow(initialY + (5f*(initialY)), 2);
//
//            object3DData.setInitialLoc(object3DData.getLocation());
//            object3DData.setLocation(new float[]{object3DData.getLocationX(), transY, object3DData.getLocationZ()});
//            object3DData.setMoved(true);
//        }
//    }
}