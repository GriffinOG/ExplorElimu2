...
class MainActivity : AppCompatActivity() {
        ...
        boundServiceConnection = object : ServiceConnection {
            override fun onServiceConnected(name: ComponentName, service: IBinder) {
                ...
                if (firstFrag is MembersFragment){
                    val userType = AccountManager.getInstance(mConnection).getAccountAttribute("name").substringAfterLast(
                        "101"
                    ).substringBeforeLast("404")
    ...
    private fun loadModelFromSdCard() {
        ...
        if (AndroidUtils.checkPermission(
                this,
                Manifest.permission.READ_EXTERNAL_STORAGE,
                REQUEST_READ_EXTERNAL_STORAGE
            )) {
            FileUtils.createChooserDialog(
                this, "Select file", null, null, SUPPORTED_FILE_TYPES_REGEX
            ) { file: File? ->
                if (file != null) {
    ...
    private fun requestPermissionForMicrophone() {
        if (ActivityCompat.shouldShowRequestPermissionRationale(
            ...
            Toast.makeText(
                this,
                com.example.render.R.string.permissions_needed,
                Toast.LENGTH_LONG
            ).show()
        } else {
